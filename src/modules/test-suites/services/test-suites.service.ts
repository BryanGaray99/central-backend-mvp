...
export class TestSuitesService {
  ...
  async createTestSuite(projectId: string, dto: CreateTestSuiteDto): Promise<TestSuiteResponseDto> {
    ...
    // Generate suite ID
    const suiteId = await this.generateSuiteId(project.name, dto.type, dto.section || '', dto.entity || '');
  ...
  async executeTestSuite(projectId: string, suiteId: string, dto?: ExecuteTestSuiteDto): Promise<{
    ...
    // Verificar si hay test cases usando el array en lugar del contador
    const actualTestCasesCount = testSuite.testCases?.length || 0;
    
    if (actualTestCasesCount === 0) {
      this.logger.warn(`Test suite ${suiteId} has no test cases to execute. This might be due to:`);
      ...
      throw new BadRequestException(`Test suite ${suiteId} has no test cases to execute. Please add test cases to this test suite before executing.`);
    }
    
    // Corregir el totalTestCases si está inconsistente
    ...
    try {
      // Extraer los nombres de los escenarios específicos de la test suite
      const specificScenarios = testSuite.testCases?.map(tc => tc.name) || [];
      
      this.logger.log(`Test suite ${suiteId} contains ${specificScenarios.length} specific scenarios: ${specificScenarios.join(', ')}`);
      
      // Preparar los datos para la ejecución usando las opciones del DTO
      const executeTestsDto = {
        ...
        testSuiteId: suiteId, // Importante: pasar el testSuiteId para linkear
      };
      ...
      this.logger.log(`Test suite ${suiteId} execution initiated with execution ID: ${executionResult.executionId}`);
      ...
      return {
        ...
        data: {
          ...
          status: 'started',
          message: `Test suite execution started with ${testSuite.totalTestCases} test cases`,
  ...
  private async generateSuiteId(projectName: string, type: TestSuiteType, section: string, entity: string): Promise<string> {
    const prefix = type === TestSuiteType.TEST_SET ? 'TS' : 'TP';
    
    // Buscar el último ID incremental para esta section-entity
